...
- Assets raise the estimate using the same asset assessment rate the federal aid formula publishes, capped at the school's actual full price so nobody is modeled as paying more than sticker price. It doesn't separately model the broader set of assets (like home equity) that CSS Profile schools weigh, since a family's assets are stored as one combined figure rather than broken out by type.
